...
namespace Helioviewer\EventsApi\Events\Sources\CCMC;
 ...
 * ```
 *
 * @package    Helioviewer\EventsApi\Collector\Sources\CCMC
...
class DonkiCmeSource extends JsonSource
{
     ...
     *
     * @throws InvalidEventException When activityID is missing or empty
     */
    public function extractRawRecordId(array $rawRecord): string
    {
        if (!isset($rawRecord['activityID']) || empty($rawRecord['activityID'])) {
            throw new InvalidEventException('DONKI CME record missing required activityID field');
        }

...
namespace Helioviewer\EventsApi\Response;
        ...
        foreach ($events as $row) {
            ...
            if (count($parts) <= 2) {
                ...
                if ($includeExtendedData) {
                    ...
                    $sourceData = $this->storage->load("/u/apps/data/sources/{$uuid}.json");
                    $eventArray['source'] = $sourceData ?: null;
                    
                    // Load views JSON data
                    $viewsData = $this->storage->load("/u/apps/data/views/{$uuid}.json");
                    $eventArray['views'] = $viewsData ?: [];
                    
                    // Load links JSON data
                    $linksData = $this->storage->load("/u/apps/data/links/{$uuid}.json");
